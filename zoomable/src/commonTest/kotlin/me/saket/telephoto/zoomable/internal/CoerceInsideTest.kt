...
class CoerceInsideTest {
  @Test fun `no zoom`() {
    ...
    assertThat(
      Rect(Offset.Zero, contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
    ).isEqualTo(Offset.Zero)

    assertThat(
      Rect(Offset.Zero.copy(x = 100f), contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
    ).isEqualTo(Offset.Zero)

    assertThat(
      Rect(Offset.Zero.copy(y = 100f), contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
    ).isEqualTo(Offset.Zero)

    assertThat(
      Rect(Offset(x = 100f, y = 100f), contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
    ).isEqualTo(Offset.Zero)
  ...
  @Test fun `horizontal movement when zoomed content is bigger than viewport size in width`() {
    ...
    assertThat(
      Rect(Offset(x = -500f, y = 0f), contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
    ).isEqualTo(Offset(x = -500f, y = 0f))

    assertThat(
      Rect(Offset(x = -1000f, y = 0f), contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
    ).isEqualTo(Offset(x = -1000f, y = 0f))

    // Shouldn't be able to go left any further.
    assertThat(
      Rect(Offset(x = -1001f, y = 0f), contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
    ).isEqualTo(Offset(x = -1000f, y = 0f))
  ...
  @Test fun `2d movement when zoomed content is bigger than viewport size in both width and height`() {
    ...
    assertThat(
      Rect(Offset(x = 10f, 20f), contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
    ).isEqualTo(Offset.Zero)

    assertThat(
      Rect(Offset(x = -250f, y = -400f), contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
    ).isEqualTo(Offset(x = -250f, y = -400f))

    assertThat(
      Rect(Offset(x = -750f, y = -600f), contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
    ).isEqualTo(Offset(x = -750f, y = -600f))

    assertThat(
      Rect(Offset(x = -1005f, y = -1007f), contentSize).topLeftCoercedInside(viewportSize, Alignment.TopCenter)
